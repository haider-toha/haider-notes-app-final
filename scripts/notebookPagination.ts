// Build-only pagination. Keep source cuts deterministic across desktop and mobile.
import type { Note } from '../types';
import type { NotebookPage } from '../components/notebookPages';
import { estimatedDiagramHeight } from '../components/notebookDiagramGeometry';
...
export function legacyPaginateNote(note: Note): NotebookPage[] {
...
function blockLines(block: string): number {
  ...
  if (/```mermaid/.test(body)) {
    // Keep each complete figure atomic and reserve its measured inline height.
    // The expanded viewer provides readable zoom for dense full-fit previews.
