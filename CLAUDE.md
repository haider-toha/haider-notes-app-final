...
Desktop page grabs must work along either outer edge without hover curls; mobile uses the explicit side grips and page-owned footer controls. The page-flip engine takes corner coordinates: preserve the grab anchor plus pointer displacement mapping. Feeding raw mid-edge positions into the engine makes a tiny grab fold half the sheet. Verify small pulls settle back, full pulls turn, and links still work after pagination changes.
...
Notebook paper details: `notebookDetails.ts` owns a small post-turn settle on the inner sheet only, changing stack depths and decorative reverse ink. Never animate the engine-owned leaf transform. The book has two actual preliminary contents leaves, numbered i and ii, followed by reading pages numbered from 1. `NotebookContents` groups all 27 notes across these leaves (14/13), with plain ink navigation and page numbers. Show the contents title on the left only; preserve an equal-height spacer on the right so groups align. A small ink control inside each page’s upper outer margin navigates to `/contents` or returns to the remembered reading spread at its canonical note URL. About me is the default landing surface even when a reading position is saved; contents remains accessible through the top-of-page link and its own route. There are no bookmark ribbons, protruding tabs or contents popups. Keep the engine's two-page contents offset separate from source-page indices used in routes, source audits and saved reading positions. The guarded reading-position key stores a note ID and source offset, never draft content; unavailable storage must not break reading.
