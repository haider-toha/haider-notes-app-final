...
Notebook paper details: `notebookDetails.ts` owns a small post-turn settle on the inner sheet only, changing stack depths and decorative reverse ink. Never animate the engine-owned leaf transform. `NotebookBookmarks` links start/work/off-duty to existing reading pages; labels reveal on hover/focus and dismiss with Escape. Only reading position is stored (`haider-notebook-place`), never draft content; unavailable storage must not break reading.
