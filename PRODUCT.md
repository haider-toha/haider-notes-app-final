...
Desktop outer-edge drags, taps, or arrow keys turn flexible sheets. Mobile uses full-height 20px side grips and 44px corner areas for folding, with a previous link and no next button. Ordinary prose gestures retain native vertical scrolling; wide equations and tables retain horizontal scrolling, and browser pinch remains available. The page stays flat on hover. Reduced motion changes pages immediately, and viewport-driven geometry changes wait until an active fold finishes.
