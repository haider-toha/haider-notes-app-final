...
Desktop outer-edge drags, taps, or arrow keys turn flexible sheets. Mobile uses explicit 20px side grips for folding, with no previous/next buttons. Ordinary prose gestures retain native vertical scrolling; wide equations and tables retain horizontal scrolling, and browser pinch remains available. The page stays flat on hover. Reduced motion changes pages immediately, and viewport-driven geometry changes wait until an active fold finishes.
